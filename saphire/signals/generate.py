...
def router_examples(rollouts: Iterable[Rollout], success_weight: float = 1.0, step_weight: float = 0.5) -> list[tuple[str, str, float]]:
    ...
    for ro in rollouts:
        ...
        for s in ro.steps:
            # query as seen by the router at that step: instruction + latest user message
            last_user = next((m.content for m in reversed(s.prompt_messages) if m.role == Role.user), "")
            query = instr + " " + (last_user if last_user != instr else "")
...
def sft_examples(rollouts: Iterable[Rollout], tools_by_env: dict[str, list[ToolSpec]] | None = None,
                 min_reward: float = 0.99) -> list[dict[str, Any]]:
    out = []
    for ro in rollouts:
        ...
        for s in ro.steps:
            out.append({"prompt": messages_to_text(s.prompt_messages, _tools_for(s, tools)), "completion": assistant_to_text(s.response),
                        "task_id": ro.task_id, "rollout_id": ro.id, "step_index": s.index, "env": ro.env_name})
    return out


def preference_pairs(rollouts: Iterable[Rollout], tools_by_env: dict[str, list[ToolSpec]] | None = None) -> list[dict[str, Any]]:
    ...
    for task_id, ros in by_task.items():
        ...
        for g in good:
            ...
            for b in bad:
                for sg, sb in zip(g.steps, b.steps):
                    ...
                    if tg != tb:
                        pairs.append({"prompt": messages_to_text(sg.prompt_messages, _tools_for(sg, tools)), "chosen": tg, "rejected": tb,
                                      "task_id": task_id, "env": g.env_name})
                        break
        ...
        for s in ro.steps:
            ...
            if key not in seen:
                ...
                out.append({"prompt": messages_to_text(s.prompt_messages, _tools_for(s, tools)), "task_id": ro.task_id, "env": ro.env_name,
                            "step_index": s.index, "prefix_calls": list(prefix), "expected_tools": task.expected.get("tools", [])})
            for tc in s.response.tool_calls:
